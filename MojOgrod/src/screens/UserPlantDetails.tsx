...
const UserPlantDetails = ({route}) => {
  ...
  async function plantTypeInfo() {
    navigation.navigate(
      'PlantTypeDetails' as never,
      {plantInfo: plantInfo} as never,
    );
  }
  ...
  const fertilizationInterval =
    plantInfo.fertilizationIntervalInWeeks.toString();
  const fMonthStart =
    plantInfo.fertilizationMonthBetweenCondition[0].toString();
    ...
    <View style={{width: '100%'}}>
      ...
      <PlantDetailsSection title={'Czas między podlewaniem:'}>
        <Text style={[styles.detailsText, {backgroundColor: chipColors.blue}]}>
          {wateringInterval} dni
        </Text>
      </PlantDetailsSection>

      <PlantDetailsSection title={'Czas między nawożeniem:'}>
        <Text style={[styles.detailsText, {backgroundColor: chipColors.blue}]}>
          {fertilizationInterval} tygodni
        </Text>
      </PlantDetailsSection>

      <PlantDetailsSection title={'Miesiące nawożenia: '}>
        <Text style={[styles.detailsText, {backgroundColor: chipColors.blue}]}>
          {monthMap.get(parseInt(fMonthStart, 10))} -{' '}
          {monthMap.get(parseInt(fMonthEnd, 10))}
        </Text>
      </PlantDetailsSection>
    </View>
  );
  ...
  const footerContents = (
    <>
      <BackButton />
      <FooterTextButton text="STATYSTYKI" onPress={onPressStats} />
